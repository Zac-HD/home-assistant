...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Optional(CONF_STATION, default=None): validate_station,
...
def _get_bom_stations():
    """Return {CONF_STATION: (lat, lon)} for all stations, making
    ...
    with io.BytesIO() as fh:
        with ftplib.FTP('ftp.bom.gov.au') as ftp:
            ...
            ftp.retrbinary('RETR stations.zip', fh.write)
        fh.seek(0)
        with zipfile.ZipFile(fh) as zipped:
            with zipped.open('stations.txt') as zf:
                for _ in range(4): zf.readline()  # skip header
                while True:
                    line = zf.readline().decode().strip()
                    if len(line) < 120:
    ...
    for st in ('nsw', 'vic', 'qld', 'wa', 'tas', 'nt'):
        url = 'http://www.bom.gov.au/{0}/observations/{0}all.shtml'.format(st)
...
def bom_stations(cache_dir):
    ...
    if not os.path.isfile(cache_file):
        ...
        with gzip.open(cache_file, 'wt') as fh:
            json.dump(stations, fh, sort_keys=True)
        return stations
